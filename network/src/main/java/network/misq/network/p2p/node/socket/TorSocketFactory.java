...
@Slf4j
public class TorSocketFactory implements SocketFactory {
    ...
    public TorSocketFactory(NetworkConfig networkConfig) {
    ...
    public Optional<Address> getServerAddress(String serverId) {
        ...
        if (new File(fileName).exists()) {
            try {
                ...
                return Optional.of(new Address(host, TorSocketFactory.DEFAULT_PORT));
            } catch (IOException e) {
                log.error(e.toString(), e);
            }
        }
