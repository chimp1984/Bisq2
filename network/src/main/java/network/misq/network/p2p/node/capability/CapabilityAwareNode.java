...
import network.misq.network.p2p.node.connection.RawConnectionListener;
 ...
 * When attempting to send a Message while the handshake is not completed the message will be kept in a queue for
 ...
 */
public class CapabilityAwareNode implements RawConnectionListener, MessageListener, Closeable {
    ...
    public CapabilityAwareNode(NetworkConfig networkConfig, MessageListener messageListener, ConnectionListener connectionListener) {
        ...
        rawNode = new RawNode(networkConfig, this);
    }
    ...
    @Override
    public void onConnection(RawConnection rawConnection) {
        setupResponseHandler(rawConnection);
    }
                ...
                .orElseGet(() -> {
                    ...
                    if (addressesOfPendingConnections.contains(peerAddress)) {
                        ...
                        return rawNode.getOrCreateConnection(peerAddress)
                                .thenCompose(rawConnection -> requestCapability(rawConnection, peerAddress))
    ...
    public SocketFactory getSocketFactory() {
        return rawNode.getSocketFactory();
    }
                ...
                capability -> {
                    if (!isStopped) {
                        log.debug("setupResponseHandler: peerAddress: {}, myAddress={}, rawConnection: {}",
                                capability.address(), myAddress, rawConnection);
    ...
    private void onConnection(Connection connection) {
        ...
        RawConnection.MessageListener messageListener = message -> CapabilityAwareNode.this.onMessage(message, connection);
